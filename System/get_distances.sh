...
echo 0 | gmx trjconv -s conf_pull.tpr -f conf_pull.xtc -o conf.gro -sep

# compute distances
for (( i=0; i<21; i++ ))
do
    gmx distance -s conf_pull.tpr -f conf${i}.gro -n index.ndx -select 'com of group "SOX" plus com of group "SH"' -oall dist${i}.xvg 
done
