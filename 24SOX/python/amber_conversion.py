...
angle_df = pd.DataFrame({'Angle': angle_names,
...
bond_df = pd.DataFrame({'Bond': bond_names,
                         'b0 (Amber)': b0_amber,
                         'k_b (Amber)': k_b_amber})
bond_df['b0 (GROMACS)'] = bond_df['b0 (Amber)']*ang_to_nm
bond_df['k_b (GROMACS)'] = bond_df['k_b (Amber)']*kcal_j*inv_ang_square_to_inv_nm_square

dihedral_names = ['n2-ce', 'n2-n']
theta_0 = [180.0, 0.0]
k_theta = [8.3, 0.8]
dihedral_df = pd.DataFrame({'Dihedral': dihedral_names,
                            'Theta (Amber)': theta_0,
                            'k_theta (Amber)': k_theta})
